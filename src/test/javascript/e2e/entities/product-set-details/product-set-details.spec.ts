...
describe('ProductSetDetails e2e test', () => {
    ...
    it('should load ProductSetDetails', async () => {
        ...
        expect(await productSetDetailsComponentsPage.getTitle()).to.eq('resourceApp.productSetDetails.home.title');
    });
    ...
    it('should load create ProductSetDetails page', async () => {
        ...
        expect(await productSetDetailsUpdatePage.getPageTitle()).to.eq('resourceApp.productSetDetails.home.createOrEditLabel');
    ...
    it('should create and save ProductSetDetails', async () => {
        ...
        expect(await productSetDetailsUpdatePage.getSubGroupNoInput()).to.eq('5');
        expect(await productSetDetailsUpdatePage.getSubGroupMinCountInput()).to.eq('5');
        expect(await productSetDetailsUpdatePage.getSubGroupMinTotalInput()).to.eq('5');
        expect(await productSetDetailsUpdatePage.getMinCountInput()).to.eq('5');
        expect(await productSetDetailsUpdatePage.getMaxCountInput()).to.eq('5');
        ...
        if (await selectedIsOptional.isSelected()) {
            ...
            expect(await productSetDetailsUpdatePage.getIsOptionalInput().isSelected()).to.be.false;
        } else {
            await productSetDetailsUpdatePage.getIsOptionalInput().click();
            expect(await productSetDetailsUpdatePage.getIsOptionalInput().isSelected()).to.be.true;
        }
        await productSetDetailsUpdatePage.save();
        expect(await productSetDetailsUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await productSetDetailsComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
    ...
    it('should delete last ProductSetDetails', async () => {
        ...
        expect(await productSetDetailsDeleteDialog.getDialogTitle()).to.eq('resourceApp.productSetDetails.delete.question');
