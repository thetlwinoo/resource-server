...
describe('Photos e2e test', () => {
    ...
    it('should load Photos', async () => {
        ...
        expect(await photosComponentsPage.getTitle()).to.eq('resourceApp.photos.home.title');
    });
    ...
    it('should load create Photos page', async () => {
        ...
        expect(await photosUpdatePage.getPageTitle()).to.eq('resourceApp.photos.home.createOrEditLabel');
    ...
    it('should create and save Photos', async () => {
        ...
        expect(await photosUpdatePage.getThumbnailPhotoInput()).to.eq('thumbnailPhoto');
        expect(await photosUpdatePage.getOriginalPhotoInput()).to.eq('originalPhoto');
        expect(await photosUpdatePage.getBannerTallPhotoInput()).to.eq('bannerTallPhoto');
        expect(await photosUpdatePage.getBannerWidePhotoInput()).to.eq('bannerWidePhoto');
        expect(await photosUpdatePage.getCirclePhotoInput()).to.eq('circlePhoto');
        expect(await photosUpdatePage.getSharpenedPhotoInput()).to.eq('sharpenedPhoto');
        expect(await photosUpdatePage.getSquarePhotoInput()).to.eq('squarePhoto');
        expect(await photosUpdatePage.getWatermarkPhotoInput()).to.eq('watermarkPhoto');
        expect(await photosUpdatePage.getThumbnailPhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getOriginalPhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getBannerTallPhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getBannerWidePhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getCirclePhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getSharpenedPhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getSquarePhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getWatermarkPhotoBlobInput()).to.endsWith(fileNameToUpload);
        expect(await photosUpdatePage.getPriorityInput()).to.eq('5');
        ...
        if (await selectedDefaultInd.isSelected()) {
            ...
            expect(await photosUpdatePage.getDefaultIndInput().isSelected()).to.be.false;
        } else {
            await photosUpdatePage.getDefaultIndInput().click();
            expect(await photosUpdatePage.getDefaultIndInput().isSelected()).to.be.true;
        }
        expect(await photosUpdatePage.getDeleteTokenInput()).to.eq('deleteToken');
    ...
    it('should delete last Photos', async () => {
        ...
        expect(await photosDeleteDialog.getDialogTitle()).to.eq('resourceApp.photos.delete.question');
