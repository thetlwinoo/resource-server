...
export class CustomerCategoriesService {
    ...
    protected convertDateFromClient(customerCategories: ICustomerCategories): ICustomerCategories {
        const copy: ICustomerCategories = Object.assign({}, customerCategories, {
            validFrom:
                customerCategories.validFrom != null && customerCategories.validFrom.isValid()
                    ? customerCategories.validFrom.format(DATE_FORMAT)
                    : null,
            validTo:
                customerCategories.validTo != null && customerCategories.validTo.isValid()
                    ? customerCategories.validTo.format(DATE_FORMAT)
                    : null
        });
        return copy;
    }
