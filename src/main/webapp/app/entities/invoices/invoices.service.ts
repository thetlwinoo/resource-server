...
export class InvoicesService {
    ...
    protected convertDateFromClient(invoices: IInvoices): IInvoices {
        const copy: IInvoices = Object.assign({}, invoices, {
            invoiceDate: invoices.invoiceDate != null && invoices.invoiceDate.isValid() ? invoices.invoiceDate.format(DATE_FORMAT) : null,
            confirmedDeliveryTime:
                invoices.confirmedDeliveryTime != null && invoices.confirmedDeliveryTime.isValid()
                    ? invoices.confirmedDeliveryTime.toJSON()
                    : null
        });
        return copy;
    }
