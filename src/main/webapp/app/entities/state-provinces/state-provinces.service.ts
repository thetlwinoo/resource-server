...
export class StateProvincesService {
    ...
    protected convertDateFromClient(stateProvinces: IStateProvinces): IStateProvinces {
        const copy: IStateProvinces = Object.assign({}, stateProvinces, {
            validFrom:
                stateProvinces.validFrom != null && stateProvinces.validFrom.isValid()
                    ? stateProvinces.validFrom.format(DATE_FORMAT)
                    : null,
            validTo: stateProvinces.validTo != null && stateProvinces.validTo.isValid() ? stateProvinces.validTo.format(DATE_FORMAT) : null
        });
        return copy;
    }
