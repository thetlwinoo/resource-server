...
export class DeliveryMethodsService {
    ...
    protected convertDateFromClient(deliveryMethods: IDeliveryMethods): IDeliveryMethods {
        const copy: IDeliveryMethods = Object.assign({}, deliveryMethods, {
            validFrom:
                deliveryMethods.validFrom != null && deliveryMethods.validFrom.isValid()
                    ? deliveryMethods.validFrom.format(DATE_FORMAT)
                    : null,
            validTo:
                deliveryMethods.validTo != null && deliveryMethods.validTo.isValid() ? deliveryMethods.validTo.format(DATE_FORMAT) : null
        });
        return copy;
    }
