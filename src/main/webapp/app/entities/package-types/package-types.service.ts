...
export class PackageTypesService {
    ...
    protected convertDateFromClient(packageTypes: IPackageTypes): IPackageTypes {
        const copy: IPackageTypes = Object.assign({}, packageTypes, {
            validFrom:
                packageTypes.validFrom != null && packageTypes.validFrom.isValid() ? packageTypes.validFrom.format(DATE_FORMAT) : null,
            validTo: packageTypes.validTo != null && packageTypes.validTo.isValid() ? packageTypes.validTo.format(DATE_FORMAT) : null
        });
        return copy;
    }
