        ...
        const copy: IColdRoomTemperatures = Object.assign({}, coldRoomTemperatures, {
            recordedWhen:
                coldRoomTemperatures.recordedWhen != null && coldRoomTemperatures.recordedWhen.isValid()
                    ? coldRoomTemperatures.recordedWhen.format(DATE_FORMAT)
                    : null,
            validFrom:
                coldRoomTemperatures.validFrom != null && coldRoomTemperatures.validFrom.isValid()
                    ? coldRoomTemperatures.validFrom.format(DATE_FORMAT)
                    : null,
            validTo:
                coldRoomTemperatures.validTo != null && coldRoomTemperatures.validTo.isValid()
                    ? coldRoomTemperatures.validTo.format(DATE_FORMAT)
                    : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((coldRoomTemperatures: IColdRoomTemperatures) => {
                coldRoomTemperatures.recordedWhen =
                    coldRoomTemperatures.recordedWhen != null ? moment(coldRoomTemperatures.recordedWhen) : null;
